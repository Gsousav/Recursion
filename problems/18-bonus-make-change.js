...
function makeBetterChange(amount, coins = [1, 5, 10, 25]) {
  ...
  const withCoin = makeBetterChange(amount - currentCoin, coins);
  if (withCoin !== null) {
    withCoin.push(currentCoin);
  }

  const withoutCoin = makeBetterChange(amount, remainingCoins);

  if (withCoin === null) {
    return withoutCoin;
  } else if (withoutCoin === null) {
    return withCoin;
  } else {
    return withCoin.length < withoutCoin.length ? withCoin : withoutCoin;
  }
}
